...
CeorlShell SHALL be the only component that updates the active index. The `useScrollSettle` hook SHALL NOT call `onIndexChange` or any equivalent that would override Shell's decision about which column is focused. When `focusColumn` is called, Shell SHALL NOT write `activeIndex` immediately — it SHALL only write `activeIndex` after the scroll settle event confirms the final position.
...
- **THEN** `useScrollSettle` SHALL report the observed index to Shell, and Shell SHALL adopt it
...
### Requirement: useScrollSettle does not mutate caller state
`useScrollSettle` SHALL NOT accept or mutate any ref that is owned by the calling component for the purpose of suppressing scroll events. The hook SHALL be a pure observer.

#### Scenario: No ignoreRef parameter exists
- **WHEN** reviewing the `useScrollSettle` function signature
- **THEN** there SHALL be no `ignoreRef` parameter

### Requirement: useScrollSettle reports settle events with sequence number
`useScrollSettle` SHALL provide an `onScrollSettle` callback option that fires when the scroll position stabilizes (via `scrollend` or debounced `scroll`). The callback SHALL receive the observed column index and a monotonic sequence number identifying the scroll gesture.
...
CeorlShell SHALL maintain an internal sequence counter. On each `focusColumn` call, the counter SHALL increment. The `onScrollSettle` callback SHALL only update `activeIndex` if its sequence matches the current counter.
