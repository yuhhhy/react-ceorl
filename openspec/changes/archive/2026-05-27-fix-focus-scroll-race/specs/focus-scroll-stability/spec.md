...
When `focusColumn` triggers a smooth scroll, the `onIndexChange` callback SHALL be called exactly once — with the final correct index after the scroll animation completes. No intermediate or incorrect index SHALL be emitted during the scroll animation.
...
- **THEN** `onIndexChange` SHALL be called with 3 at the end of the animation, and SHALL NOT be called with 0 or any other index during the animation
